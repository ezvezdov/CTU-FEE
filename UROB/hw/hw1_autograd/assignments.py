...
def dkt_vectorized(omega: list, rho: list, base: np.ndarray) -> np.ndarray:
    ...
    # -------------------------------------------------
    # START OF YOUR CODE
    lower_triangular = Tensor(np.tri(n, n, dtype=int))
    phi = lower_triangular @ omega

    cos_phi = phi.cos()
    sin_phi = phi.sin()
    cos_sin_phi = cos_phi.stack(sin_phi)

    rho_diag = Tensor(np.diag(rho.data))
    x_prime = cos_sin_phi @ rho_diag

    upper_triangular = Tensor(np.tri(n, n, dtype=int).T)

    X = x_prime @ upper_triangular
...
def manipulator_loss(X: np.ndarray, x_goal: np.ndarray) -> Tensor:
    ...
    loss = (X[:,-1][0] - x_goal[0])**2 + (X[:,-1][1] - x_goal[1])**2
