...
class MyNet:
    ...
    def __init__(self, input_size, hidden_size,reg=1e-6):
        self.name = f'test-net-{hidden_size}' # name is needed for printing
    ...
    def score(self, x: np.array) -> np.array:
        ...
        """
        s = self.lifting(x) @ self.w + self.b
        s = s.reshape(-1)
        return s
    ...
    def get_loss(self, data: np.array):
        """
        Get value of the loss function
        *
        :param data: tuple (x,y) of data arrays: x[N x 2], y[Y]
        :return: l: scalar, loss value
        """
        x = data[0]
        y = data[1]
        s = self.score(x)
        loss = np.mean(np.linalg.norm(s - y)**2)
        return loss
    ...
    def train(self, train_data):
        ...
        *
        ...
        theta = np.linalg.inv(Phi.T @ Phi + self.reg * np.eye(Phi.shape[1],Phi.shape[1])) @ Phi.T @ y        
        self.w = np.reshape(theta[:-1],(theta[:-1].shape[0],1))
        self.b = theta[-1]
