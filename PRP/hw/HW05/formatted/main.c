...
int Levenshtein(char *str1, char *str2, int len1, int len2) {
  ...
  for (int i = 1; i <= len1; i++) {
    for (int j = 1; j <= len2; j++) {
      arr[i * (len2 + 1) + j] =
          min(min(arr[i * (len2 + 1) + j - 1], arr[(i - 1) * (len2 + 1) + j]),
              arr[(i - 1) * (len2 + 1) + j - 1]) +
          !(str1[i - 1] == str2[j - 1]);
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if ((len1 != len2 && argc == 1) ||
      (len1 != len2 && strcmp(argv[1], optional_argument))) {
    free(str1);
    free(str2);
  ...
  for (int i = 0; i < SHIFT_COUNT; i++) {
    ...
    if ((len1 == len2 && current_compare > best_compare) ||
        (len1 != len2 && current_compare < best_compare)) {
