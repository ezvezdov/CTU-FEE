...
def matrix_manip(A, B):
    ...
    output['A_3rd_col'] = A[:,2]
    output['A_3rd_col'] = output['A_3rd_col'][:,np.newaxis]
    output['A_slice'] = A[-2:,-3:]
    # output['A_gr_inc'] = np.ones((A.shape[0],A.shape[1]+1))
    # output['A_gr_inc'][:,:-1] = A + (A > 3).astype(np.int64)
    output['A_gr_inc'] = A + (A > 3).astype(A.dtype)
    output['A_gr_inc'] = np.c_[output['A_gr_inc'],np.ones(A.shape[1],dtype=A.dtype)]
    output['C'] = output['A_gr_inc'] @ output['A_gr_inc'].T
    
    output['A_weighted_col_sum'] = float(np.sum(np.arange(1,output['A_gr_inc'].shape[1]+1) * np.sum(output['A_gr_inc'],axis = 0)))
    output['D'] = B - np.array([[4,6]]).T

    euclidean_lengths = np.linalg.norm(output['D'],axis=0)
    mean_length = np.mean(euclidean_lengths)
    

    output['D_select'] = output['D'][:,euclidean_lengths > mean_length]
    return output
...
def compute_letter_mean(letter_char, alphabet, images, labels):
    ...
    """
    idx = np.where(alphabet == letter_char)[0][0]
    letter_imgs = images[:,:, idx == labels]
    letter_mean = np.round(np.mean(letter_imgs,axis=2))
...
def compute_lr_features(letter_char, alphabet, images, labels):
    ...
    idx = np.where(alphabet == letter_char)[0][0]
    letter_imgs = images[:,:, idx == labels]
    lr_features = np.int32(np.sum(letter_imgs[:,:5,:],axis=(0,1))) - np.int32(np.sum(letter_imgs[:,5:,:],axis=(0,1)))
    return lr_features
